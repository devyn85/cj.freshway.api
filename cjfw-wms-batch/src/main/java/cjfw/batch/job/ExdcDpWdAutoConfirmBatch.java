...
public class ExdcDpWdAutoConfirmBatch {

	private static final String BATCH_JOB_NAME = "exdcDpWdAutoConfirmJob";
	...
	@Bean
	public Job exdcDpWdAutoConfirmJob() {
	...
	public Tasklet execTasklet() {
		return (contribution, chunkContext) -> {
			...
			try {
				...
				commonDao.selectOne("exdcDpWdAutoConfirmService.callProcExecute", batchParam);
			} catch (Exception e) {
				batchLogDto.setResultCode("-1");
				batchLogDto.setResultMsg(e.toString());
			} finally {
				batchLogDto.setJobStatus("END");
				batchLogDto.setLineNo("91");
				commonDao.selectOne("batchCommonService.insertBatchLog", batchLogDto);
			}
