...
public class Scm0110BatchService {
	...
	public void callScm0110JobService(BatchParamsUtil batchParam) throws JobExecutionException {
		callScm0110JobService(batchParam, null);
	}
	
	@Transactional
	public void callScm0110JobService(BatchParamsUtil batchParam, BatchLogParamsDto batchLogDto) throws JobExecutionException {
		//BATCH JOB LOG설정
		...
		if( ifStaus.equals("END") ) {
			try {
				List<Scm0110ParamsDto> sendList =  commonDao.selectList("scm0110Service.selectScm0110Header");
				logSystemOut(copyDto, "INFO", logOption[4], "69", "[sqlId]selectScm0110Header Rows:"+String.valueOf(sendList.size()));
				...
				for(Scm0110ParamsDto sendData : sendList) {
					...
					try {
						...
						if(errorCode == 0) {
							...
							logSystemOut(copyDto, "INFO", logOption[4], "123", "[sqlId]selectScm0110Detail Rows:"+String.valueOf(detailSearchList.size()));
							List<Scm0110ParamsDto> serialSearchList = commonDao.selectList("scm0110Service.selectScm0110Serial", inputParam);
							logSystemOut(copyDto, "INFO", logOption[4], "125", "[sqlId]selectScm0110Serial Rows:"+String.valueOf(serialSearchList.size()));
				...
				logSystemOut(copyDto, "INFO", logOption[4], "268", e.getMessage());
			}
		}
		...
		//BATCH JOB LOG설정
		logSystemOut(copyDto, "END", logOption[4], "285", "JAVA SERVICE END");
	}
