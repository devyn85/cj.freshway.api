package cjfw.wms.ms.dto;

import io.swagger.v3.oas.annotations.media.Schema;
import lombok.Data;

/**
 * Copyright 2025. CJ OliveNetworks Co. all rights reserved.
 * @author : JeongHyeongCheol (wjdgudcjf@cj.net) 
 * @date : 2025.08.04 
 * @description : 센터이체마스터 피킹유형 자동 설정 응답 DTO
 * @issues :<pre> 
 * ----------------------------------------------------------- 
 * DATE       AUTHOR                MAJOR_ISSUE 
 * ----------------------------------------------------------- 
 * 2025.08.04 JeongHyeongCheol (wjdgudcjf@cj.net) 생성 </pre>
 */
@Data
public class MsCenterStoPickResDto {
	
	/** 데이터번호 */
	@Schema(description = "데이터번호", example = "")
	private String serialkey;
	
	/** 센터코드 */
	@Schema(description = "센터코드", example = "")
	private String dccode;
	
	/** 센터마감유형 */
	@Schema(description = "센터마감유형", example = "")
	private String dcClosetype;
		
	/** 그룹유형 */
	@Schema(description = "그룹유형", example = "")
	private String groupType;
	
	/** 피킹유형 */
	@Schema(description = "피킹유형", example = "")
	private String pickedType;
		
	/** 적용여부 */
	@Schema(description = "적용여부", example = "")
	private String applyYn;	

	/** 피킹유형 자동 설정 유효성 */
	@Schema(description = "피킹유형 자동 설정 유효성", example = "")
	private Integer pickListChk;
	
}