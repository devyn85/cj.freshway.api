 ...
 */
@Getter
public enum TmDispatchExcelColumn {

	
	NO("no", "순서", "순서", 1, STRING, FIXED, CENTER, false, false),	
    // 물류센터
	DCCODE("dccode", "물류센터코드", "물류센터", 2, STRING, FIXED, LEFT, false, true),
    DCNAME("dcname", "물류센터", "물류센터", 3 , STRING, FIXED, LEFT, false, false),
     
    // 배송정보 
    DELIVERY_DATE("deliveryDate", "배송일자", "배송정보", 4, STRING, FIXED, LEFT, false, true),
    DELIVERY_TYPE("deliveryType", "배송유형", "배송정보", 5, STRING, EDITABLE, CENTER, false, false),
    ...
    ZIP_CODE("zipCode", "우편번호", "권역정보", 9, STRING, FIXED, CENTER, false, false),
   
    // 클레임
    CLAIM_YN("claimYn", "클레임", "클레임", 10, STRING, FIXED, CENTER, false, false),
    
    // POP
    POP_NAME("popName", "POP", "POP", 11, STRING, FIXED, LEFT, false, false),
    
    // 고객정보
    TRUTH_CUST_KEY("truthCustKey", "실착지코드", "고객정보", 12, STRING, FIXED, CENTER, false, false),
    CUST_NAME("custName", "실착지명", "고객정보", 13, STRING, FIXED, LEFT, false, false),
    CUST_KEY("custKey", "관리처코드", "고객정보", 14, STRING, FIXED, CENTER, false, false),
    FROM_CUST_NAME("fromCustName", "관리처명", "고객정보", 15, STRING, FIXED, LEFT, false, false),
    ...
    TURN("turn", "회차", "차량정보", 19, STRING, FIXED, CENTER, false,  true),
    ...
    // 물량 (천단위 구분 기호 사용)
    TOTAL_WEIGHT("totalWeight", "중량(kg)", "물량", 23, DECIMAL, FIXED, RIGHT, true, false),
    TOTAL_CUBE("totalCube", "체적(m³)", "물량", 24, DECIMAL, FIXED, RIGHT, true, false),

    // 배송조건
    FACE_CHECK_YN("faceCheckYn", "대면검수여부", "배송조건", 25, STRING, FIXED, CENTER, false, false),
    OTD("otd", "OTD", "배송조건", 26, STRING, FIXED, CENTER, false, false),
    SPECIAL_CONDITION_YN("specialConditionYn", "특수조건", "배송조건", 27, STRING, FIXED, CENTER, false, false),
    ...
    // 주소
    ADDRESS("address", "주소", "고객정보", 30, STRING, FIXED, LEFT, false, false);
