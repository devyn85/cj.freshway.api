...
public class DailyInoutCloseBatch {

	private static final String BATCH_JOB_NAME = "dailyInoutCloseJob";
	...
	public Tasklet execTasklet() {
		return (contribution, chunkContext) -> {
			...
			try {
				...
				commonDao.selectOne("dailyInoutCloseService.callProcExecute", batchParam);
			} catch (Exception e) {
				batchLogDto.setResultCode("-1");
				batchLogDto.setResultMsg(e.toString());
			} finally {
				batchLogDto.setJobStatus("END");
				batchLogDto.setLineNo("98");
				commonDao.selectOne("batchCommonService.insertBatchLog", batchLogDto);
			}
