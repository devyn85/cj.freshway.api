 ...
 * @date : 2025.07.31
 * @description :재고일괄이동(수원3층)  Service Class
 * @issues :<pre>
 * -----------------------------------------------------------
...
public class StLocMoveBoxService {
	/**
	 * Use this prefix to explicitly indicate a workspace name with a query id.
	 * @return .sqlx returns the location
	 */
	private transient static final String SERVICEID_PREFIX = StringUtils.uncapitalize(StLocMoveBoxService.class.getSimpleName()) + ".";
	private transient static final String PAKAGE_NAME = "SPST_MOVEMENT";
	/** 프로세스타임 */
	private transient static final String PROCESSTYPE = "ST_BATCHMOVE_BOX";	
	/** 임시테이블 타입 */
	...
	*/
	 ...
	 */
	public StLocMoveBoxResDto saveMasterList(StLocMoveBoxReqDto paramDto) throws Exception {
		StLocMoveBoxResDto resultDto = new StLocMoveBoxResDto();
		...
		StLocMoveBoxReqDto reqDto = ModelMapperUtil.map(paramDto, StLocMoveBoxReqDto.class);
		List<StLocMoveBoxResDto> saveList = reqDto.getSaveList(); // 저장리스트
		...
		//if("".equals(cjfw.core.utility.StringUtil.nvl(reqDto.getReasonmsg()))) throw new UserHandleException( MessageUtil.getMessage("MSG_COM_ERR_056",new String[]{"requestmsg"} ) + reqDto.getReasonmsg() ); // 해당 정보가 없어 처리할 수 없습니다.-{0}
		// END.필수입력
		...
		for (int i = 0; i < saveList.size(); i++) { 
			StLocMoveBoxResDto dto = saveList.get(i);
        	// 임시테이블에 등록(2/3)
    		...
    		if("".equals(cjfw.core.utility.StringUtil.nvl(entity.getReasoncode()))      ) throw new UserHandleException( MessageUtil.getMessage("MSG_COM_ERR_056",new String[]{"requestcode"} ) ); // 해당 정보가 없어 처리할 수 없습니다.-{0}
    		if("".equals(cjfw.core.utility.StringUtil.nvl(entity.getReasonmsg()))       ) throw new UserHandleException( MessageUtil.getMessage("MSG_COM_ERR_056",new String[]{"requestmsg"} ) ); // 해당 정보가 없어 처리할 수 없습니다.-{0}
    		
    		
    		//if (entity.getToOrderqty() == null                         ) throw new UserHandleException(MessageUtil.getMessage("MSG_COM_ERR_056", new String[]{"toOrderqty"}));
    		if (entity.getToOrderqtyBox() == null) entity.setToOrderqtyBox("0");
    		if (entity.getToOrderqtyEa() == null) entity.setToOrderqtyEa("0");
    		if (entity.getToOrderqty() == null) entity.setToOrderqty(BigDecimal.ZERO);
    		if (entity.getToConfirmqty() == null) entity.setToConfirmqty(BigDecimal.ZERO);
        ...
        /*START.PAKAGE 호출*/
		// PKG 파라마터 세팅 - 공통(1/4)
		StLocMoveBoxReqDto dto = new StLocMoveBoxReqDto();
		...
		if(!"0".equals(resultCode)){
			log.error("▶재고일괄이동(수원3층) - 저장 시 오류 발생 ");
			throw new UserHandleException( MessageUtil.getMessage("MSG_COM_ERR_055",new String[]{"재고일괄이동(수원3층)-저장"}) + resultMessage ); // {0} 처리 시 문제가 발생했습니다.
		}  	
		 /*END.PAKAGE 호출*/     
